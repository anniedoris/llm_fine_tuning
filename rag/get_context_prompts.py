...
def load_output_csv(model, question_type, overwrite_answers=False):
    ...
    if not os.path.exists(csv_name) or overwrite_answers:
        questions_pd = pd.read_csv(f"../datasets/rule_{question_type}_qa.csv")
...
def create_index():
    ...
    pdf_path = "../datasets/FSAE_Rules_2024_V1.pdf"
...
if __name__ == '__main__':
    ...
    context_prompt_df = pd.DataFrame(columns=["prompt_without_context", "prompt_with_context", "ground_truth"])
        ...
        for model in ['llava-13b']:
            ...
            for i, row in tqdm(questions_pd.iterrows(), total=len(questions_pd), desc=f'generating responses for '
                ...
                #     context = retrieve_context(index, question, top_k=0)
                    ...
                    raise ValueError("Invalid model")
                try:
                    extended_prompt = add_context_to_prompt(question, context)
                    row = {'prompt_without_context': question, 'prompt_with_context':extended_prompt, 'ground_truth':row['ground_truth']}
                    context_prompt_df = pd.concat([context_prompt_df, pd.DataFrame([row])], ignore_index=True)
